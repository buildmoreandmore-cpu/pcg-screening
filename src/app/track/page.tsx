    ...
    <div className="min-h-dvh bg-[#f8f7f4]">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-5 py-6 sm:py-10 space-y-6 sm:space-y-8">
          ...
          <div className="space-y-4 animate-[fadeIn_0.3s_ease]">
            ...
            <div className="bg-white rounded-2xl overflow-hidden" style={{ boxShadow: '0 4px 16px rgba(31,47,74,0.08), 0 2px 4px rgba(31,47,74,0.04)' }}>
              ...
              <div className="p-5 sm:p-8 space-y-4">
                ...
                <div className="flex items-center justify-between py-3">
                  <span className="text-sm text-[#8a8680] uppercase tracking-wider font-medium">Package</span>
                  <span className="text-base font-medium text-[#4a4743]">{result.packageName}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#f5ecd4] rounded-2xl px-5 sm:px-6 py-4 sm:py-5 flex items-start gap-3 sm:gap-4">
              ...
              <div>
                <p className="text-sm text-[#4a4743] font-medium">Results are delivered directly to your employer.</p>
                <p className="text-sm text-[#8a8680] mt-1">Most screenings complete within 1-3 business days.</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="pt-4 pb-8">
          <div className="bg-white rounded-2xl p-6 text-center" style={{ boxShadow: '0 1px 3px rgba(31,47,74,0.06)' }}>
            <p className="text-sm text-[#8a8680] mb-3">Need help with your screening?</p>
  ...
  return (
    <div className="flex gap-5">
      ...
      <div className="flex flex-col items-center">
        <div className={`w-6 h-6 rounded-full border-[2.5px] shrink-0 transition-all ${
          complete
            ? 'bg-[#c9a44c] border-[#c9a44c]'
            : 'bg-white border-[#e2e0db]'
        } ${active ? 'ring-[8px] ring-[#c9a44c]/15 scale-110' : ''}`}>
          {complete && (
            <svg className="w-full h-full text-white p-[2px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
            </svg>
          )}
        </div>
        {!last && (
          <div className={`w-0.5 flex-1 min-h-[48px] ${complete ? 'bg-[#c9a44c]' : 'bg-[#e2e0db]'}`} />
        )}
      </div>

      {/* Content */}
      <div className={`pb-8 ${last ? 'pb-0' : ''}`}>
        <p className={`text-base font-semibold ${complete ? 'text-[#1f2f4a]' : 'text-[#c8c5be]'}`}>
          {title}
        </p>
        <p className={`text-sm mt-0.5 ${complete ? 'text-[#8a8680]' : 'text-[#c8c5be]'}`}>
          {subtitle}
        </p>
        {date && (
          <p className="text-xs text-[#c9a44c] font-medium mt-1.5">{date}</p>
        )}
      </div>
    </div>
  )
