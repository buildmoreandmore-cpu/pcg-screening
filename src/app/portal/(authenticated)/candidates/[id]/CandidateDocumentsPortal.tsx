...
/**
 * Client-side documents card. Mirrors the admin's CandidateDocuments
 * but only exposes downloads once the screening is fully complete —
 * draft state shouldn't leak to the employer.
 */
export default function CandidateDocumentsPortal({
  candidateId,
  consentDocUrl,
  reportSentAt,
  reportAttachments,
  ...
  if (isCompleted) {
    items.push({
      ...
      sublabel: reportSentAt
        ? `Sent ${new Date(reportSentAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`
        : 'Download the final report',
      href: `/api/portal/candidates/${candidateId}/report?preview=false`,
      primary: true,
    })
    items.push({
      key: 'report-preview',
      ...
      href: `/api/portal/candidates/${candidateId}/report?preview=true`,
    })
    ...
    for (const att of reportAttachments) {
      items.push({
        ...
        href: `/api/portal/candidates/${candidateId}/attachments/${encodeURIComponent(att.id)}`,
      })
    }
  }
  ...
  if (hasSignedConsent && consentDocUrl) {
    items.push({
      ...
      sublabel: `${candidateId.slice(0, 8)} signature record`,
      href: consentDocUrl,
    })
  }
    ...
    <div className="bg-white rounded-xl shadow-sm p-5">
      ...
      {!isCompleted && items.length === 0 && (
        <p className="text-sm text-gray-400">
          Documents will be available here once the screening is complete.
        </p>
      )}
      {!isCompleted && items.length > 0 && (
        <p className="text-xs text-gray-400 mb-2">
          Final report and supporting documents will appear once the screening is complete.
        </p>
      )}
